...
export default {
  ...
  computed: {
    modelValue() {
      return this.multiple && this.isString ? String(this.value).split(',').filter(i => i) : this.value;
    }
  },
  watch: {
    optionList(val = []) {
      this.listData = this.getOptionsListByFilterOp(val);
    }
  },
  created() {
    // 如果有请求，根据配置的请求获取数据
    ...
    getOptionsListByFilterOp(list) {
      ...
      list = list.map(item => {
        return {
          ...
          value: this.multiple && this.isString ? String(item[this.valueField]) : item[this.valueField]
        };
      });
    ...
    onChange(val) {
      if (this.multiple && this.isString) {
        if (this.labelInValue) {
          this.$emit(
            'input',
            val.reduce((acc, cur) => (acc + (acc === '' ? cur.value : ',' + cur.value)), '')
          );
        } else {
          this.$emit('input', Array.isArray(val) ? String(val) : val);
        }
      } else {
